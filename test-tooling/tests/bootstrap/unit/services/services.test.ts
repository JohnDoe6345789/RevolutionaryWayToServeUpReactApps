...
// Create mock service registry helper
...
describe("Services", () => {
  describe("EnvInitializer", () => {
    ...
    describe("ensureProxyMode method", () => {
      beforeEach(() => {
        // Initialize instance to set up global property
  ...
  describe("ServiceRegistryInstance", () => {
    ...
    it("should handle service registration and retrieval", () => {
      ...
      serviceRegistry.register("testService", mockService, { type: "test" }, []);
    ...
    it("should handle metadata operations", () => {
      ...
      const metadata = { version: "1.0", type: "test" };
      
      serviceRegistry.register("testService", mockService, metadata, []);
      
      expect(serviceRegistry.getMetadata("testService")).toEqual(metadata);
      expect(serviceRegistry.listServices()).toContain("testService");
    });

    it("should handle dependency validation", () => {
      const mockService = { name: "dependentService" };
      const dependencyService = { name: "dependency" };
      
      // Register dependency first
      serviceRegistry.register("dependency", dependencyService, {}, []);
      
      // Register dependent service
      serviceRegistry.register("dependentService", mockService, {}, ["dependency"]);
      
      expect(serviceRegistry.getService("dependentService")).toBe(mockService);
    });
    ...
    it("should throw for missing dependencies", () => {
      ...
      expect(() => {
        serviceRegistry.register("dependentService", mockService, {}, ["missingDependency"]);
      }).toThrow("Required services are not registered: missingDependency");
    });
