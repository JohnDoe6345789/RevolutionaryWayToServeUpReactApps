...
This folder holds docs for the configuration helpers that populate provider fallbacks, logging directives, and loader settings consumed by the bootstrap runtime. Configs now live under `core`, `cdn`, `local`, and `helpers` subfolders in `bootstrap/configs` (helpers configs are not yet documented here).
