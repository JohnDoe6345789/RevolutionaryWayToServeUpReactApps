#!/usr/bin/env python3
"""Download and cache the latest act release binary."""
...
def _select_asset(release: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(assets, list):
        raise ActBinaryError("Release metadata missing expected asset list.")
...
def _fetch_latest_release() -> dict[str, Any]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            if resp.status != 200:
                raise ActBinaryError("Unexpected response while fetching act release metadata.")
